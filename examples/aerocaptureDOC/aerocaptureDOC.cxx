...
void rhs(const vector<double>& y, vector<double>& dydt, const double t, Constants_& CONSTANTS, double bank) {
   ...
   double p0    = y[ 6 ];
    ...
    double s53 = - (sin(gamma)*(v*v/r - mu/(r*r)))/v -  eta*k*1e2* (Cl*S_ref*p0*v*exp(B*(Re + h_0 - r))*sin(bank))/(2*m);
                     ...
                     adouble& time, adouble* xad, int iphase, Workspace* workspace)
{
    double q = 1e-6;
    double r = 1e6;
    adouble runCost = q*( states[14]*states[14]+states[18]*states[18]) + r *(controls[1]*controls[1] + controls[2]*controls[2] +controls[3]*controls[3]);
    return  runCost;
}
//states[10]*states[10] + + 
// //////////////////////////////////////////////////////////////////////////
...
// {
//     double r = 1e4;
//      adouble runCost = r *(controls[1]*controls[1] + controls[2]*controls[2] +controls[3]*controls[3]);
//      return  runCost;
// }
...
////////////////////////////////////////////////////////////////////////////
    ...
    double k1_min  =   -1e-4; // -1e-5;
    double k1_max  =   1e-4;  // 1e-5;
    double k2_min  =   -1e-4; // -1e-5;
    double k2_max  =   1e-4;  // 1e-5;
    double k3_min  =   -1e-4; // -1e-5;
    double k3_max  =   1e-4;  //1e-5;
    ...
    problem.phases(iphase).bounds.lower.EndTime    = 25.0;
    problem.phases(iphase).bounds.upper.EndTime    = 170.0; 
    ...
    problem.phases(iphase).bounds.lower.StartTime    = 25.0;
    problem.phases(iphase).bounds.upper.StartTime    = 175.0;
...
////////////////////////////////////////////////////////////////////////////
    ...
    u_guess.row(3)  =1e-4*ones(1,nnodes);
