# Ubuntu 24.04 LTS.
#
# The previous long term support release, still supported and still what a
# large part of the user base is running.  It is in the matrix because the
# interesting failures are the ones that appear on the OLDER toolchain, not the
# newer one: PSOPT requires C++17 and a compiler that is two years behind is
# where a use of something later shows up.
#
