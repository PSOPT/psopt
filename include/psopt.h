...
struct alg_str {
  ...
  string    ir_residual_scaling;    // robust-DAIR box: "state" (default) non-dimensionalises each
  ...
  // "Hermite-Simpson". The global pseudospectral methods (Legendre, Chebyshev, Radau,
  // Gauss) refine through the hp driver in hp_refine.cxx and ignore it entirely, so
  // setting it on one of those has no effect whatsoever.
  int       switch_order;
...
typedef struct {
    unique_ptr<MatrixXd> dfdx_j;
    unique_ptr<MatrixXd> F1;
    unique_ptr<MatrixXd> F2;
    unique_ptr<MatrixXd> F3;
    unique_ptr<MatrixXd> F4;
    double*  x;
    double*  g;
    adouble* xad;
    adouble* gad;
...
public:
   ...
   unique_ptr<RowVectorXi[]>  sindex;
   unique_ptr<MatrixXd[]>  w;
   unique_ptr<MatrixXd[]>  D;
   MatrixXd*  D2;
   ...
   unique_ptr<MatrixXd[]>  e;
   unique_ptr<MatrixXd[]>  hgg;
   unique_ptr<MatrixXd>  Xip;
   unique_ptr<MatrixXd>  JacRow;
   unique_ptr<MatrixXd>  Gip;
   unique_ptr<MatrixXd>  GFip;
   unique_ptr<TripletSparseMatrix>  Ax;
   TripletSparseMatrix*  As;
   unique_ptr<TripletSparseMatrix>  Gsp;
   ...
   unique_ptr<MatrixXd>  constraint_scaling;
   unique_ptr<MatrixXd>  linkage;
   unique_ptr<MatrixXd[]>  prev_states;
   unique_ptr<MatrixXd[]>  prev_costates;
   unique_ptr<MatrixXd[]>  prev_controls;
   unique_ptr<MatrixXd[]>  prev_param;
   unique_ptr<MatrixXd[]>  prev_path;
   unique_ptr<MatrixXd[]>  prev_nodes;
   unique_ptr<MatrixXd>  prev_t0;
   unique_ptr<MatrixXd>  prev_tf;
   unique_ptr<MatrixXd[]>  Xdot;
   unique_ptr<MatrixXd>  JacCol1;
   unique_ptr<MatrixXd>  JacCol2;
   unique_ptr<MatrixXd>  JacCol3;
   unique_ptr<MatrixXd>  xp;
   unique_ptr<MatrixXd[]>  emax_history;
   unique_ptr<MatrixXd[]>  order_reduction;
   unique_ptr<MatrixXd[]>  old_relative_errors;
   unique_ptr<MatrixXd[]>  error_scaling_weights;


   double    obj_scaling;
   unique_ptr<GRWORK>   grw;
   int       nvars;
   int       ncons;
   int       **current_nodes;
   int       jac_done;
   unique_ptr<int[]>  iArow;
   unique_ptr<int[]>  jAcol;
   unique_ptr<int[]>  iGrow;
   unique_ptr<int[]>  jGcol;
   double*   jac_values;
   ...
   unique_ptr<unique_ptr<adouble[]>[]>  controls;
   unique_ptr<unique_ptr<adouble[]>[]>  parameters;
   unique_ptr<unique_ptr<adouble[]>[]>  resid;
   unique_ptr<unique_ptr<adouble[]>[]>  derivatives;
   unique_ptr<unique_ptr<adouble[]>[]>  initial_states;
   unique_ptr<unique_ptr<adouble[]>[]>  final_states;
   unique_ptr<unique_ptr<adouble[]>[]>  initial_controls;
   ...
   unique_ptr<adouble[]>  linkages;
   unique_ptr<adouble[]>  fgad;
   unique_ptr<adouble[]>  time_array_tmp;
   unique_ptr<adouble[]>  single_trajectory_tmp;
   unique_ptr<adouble[]>  L_ad_tmp;
   unique_ptr<adouble[]>  u_spline;
   unique_ptr<adouble[]>  z_spline;
   unique_ptr<adouble[]>  y2a_spline;
   unique_ptr<unique_ptr<adouble[]>[]>  states_next;
   unique_ptr<unique_ptr<adouble[]>[]>  controls_next;
   unique_ptr<unique_ptr<adouble[]>[]>  derivatives_next;
   unique_ptr<unique_ptr<adouble[]>[]>  path_next;
   unique_ptr<unique_ptr<adouble[]>[]>  path_bar;
   unique_ptr<unique_ptr<adouble[]>[]>  states_bar;
   unique_ptr<unique_ptr<adouble[]>[]>  controls_bar;
   unique_ptr<unique_ptr<adouble[]>[]>  derivatives_bar;
   unique_ptr<unique_ptr<adouble[]>[]>  observed_variable;
   unique_ptr<unique_ptr<adouble[]>[]>  observed_residual;
   unique_ptr<unique_ptr<adouble[]>[]>  lam_resid;
   unique_ptr<unique_ptr<adouble[]>[]>  interp_states_pe;
   unique_ptr<unique_ptr<adouble[]>[]>  interp_controls_pe;
   unique_ptr<double[]>  lambda_d;
   unique_ptr<double[]>  fg;
   bool       trace_f_done;
   unique_ptr<IGroup>    igroup;
   char       text[2000];
