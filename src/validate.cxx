...
void validate_user_input(Prob& problem, Alg& algorithm, Workspace* workspace)
{
    ...
    for (i=0;i<problem.nphases;i++)
    {
         ...
         {
            if ( algorithm.collocation_method != "Radau" && algorithm.collocation_method != "Gauss" )
            {
               snprintf(workspace->text,sizeof(workspace->text),"hp-adaptive mesh (hp_orders) in phase %i currently requires collocation_method = \"Radau\" or \"Gauss\"",i+1);
               error_message(workspace->text);
            }
